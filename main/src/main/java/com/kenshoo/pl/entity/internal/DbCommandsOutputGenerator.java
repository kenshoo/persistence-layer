...
public class DbCommandsOutputGenerator<E extends EntityType<E>> implements OutputGenerator<E> {
    ...
    private void populateGeneratedIdsToContext(Collection<? extends EntityChange<E>> entityChanges, ChangeContext changeContext, ChangesContainer changesContainer) {
        final Identity<Record, ?> autoGeneratedId = entityType.getPrimaryTable().getIdentity();
        final EntityField<E, Object> idField = (EntityField<E, Object>) entityType.findField(autoGeneratedId.getField()).get();

        seq(entityChanges)
                .map(change -> ImmutablePair.of(change, changesContainer.getInsert(entityType.getPrimaryTable(), change)))
                .filter(pair -> pair.getRight().isPresent())
                .forEach(pair -> {
                            Object generatedValue = pair.getRight().get().get(autoGeneratedId.getField());
                            changeContext.getEntity(pair.getLeft()).set(idField, generatedValue);
                });
    }
