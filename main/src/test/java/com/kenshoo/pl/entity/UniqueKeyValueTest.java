...
public class UniqueKeyValueTest {
    ...
    public void test_concat_ids_removes_duplicate_fields() {
        Identifier<TestEntity> id1 = identifier(uniqueKey(FIELD_1, FIELD_2), "1", "2");
        Identifier<TestEntity> id2 = identifier(uniqueKey(FIELD_3, FIELD_2), 3, "2");

        Identifier<TestEntity> merged = concat(id1, id2);

        assertThat(merged, is(identifier(uniqueKey(FIELD_1, FIELD_2, FIELD_3), "1", "2", 3)));
    }

    @Test
    public void test_id_concatenated_to_itself_equals_itself() {
        Identifier<TestEntity> id = identifier(uniqueKey(FIELD_1), "1");
        assertThat(concat(id, id), is(id));
    }

    private UniqueKey<TestEntity> uniqueKey(EntityField<TestEntity, ?>... fields) {
