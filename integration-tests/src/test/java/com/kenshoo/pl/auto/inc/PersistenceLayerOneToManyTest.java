...
public class PersistenceLayerOneToManyTest {
    ...
    @Test
    public void upsert_non_existing_parent_and_non_existing_children() {

        Seq.range(1, 4).forEach(this::insertSomeChildWithOrdinal);

        final UpsertByIdInTarget parent1 = new UpsertByIdInTarget(1000).with(ParentEntity.NAME, "parent1")
                .with(new UpsertChild(1).with(ChildEntity.FIELD_1, "p1_child1"))
                .with(new UpsertChild(2).with(ChildEntity.FIELD_1, "p1_child2"))
                .with(new UpsertChild(3).with(ChildEntity.FIELD_1, "p1_child3"))
                ;

        final UpsertByIdInTarget parent2 = new UpsertByIdInTarget(2000).with(ParentEntity.NAME, "parent2")
                .with(new UpsertChild(1).with(ChildEntity.FIELD_1, "p2_child1"))
                .with(new UpsertChild(2).with(ChildEntity.FIELD_1, "p2_child2"))
                .with(new UpsertChild(3).with(ChildEntity.FIELD_1, "p2_child3"))
                ;

        var retrievedParentIds = seq(persistenceLayer.upsert(List.of(parent1, parent2), flow.build()))
                .map(res -> res.getCommand().get(ParentEntity.ID)).toList();

        var childrenByNames = jooq.select().from(CHILD_TABLE).fetchMap(CHILD_TABLE.field1, toChildPojo());

        assertThat(childrenByNames.get("p1_child1").parentId, is(retrievedParentIds.get(0)));
        assertThat(childrenByNames.get("p1_child1").ordinal, is(1));

        assertThat(childrenByNames.get("p1_child2").parentId, is(retrievedParentIds.get(0)));
        assertThat(childrenByNames.get("p1_child2").ordinal, is(2));

        assertThat(childrenByNames.get("p1_child3").parentId, is(retrievedParentIds.get(0)));
        assertThat(childrenByNames.get("p1_child3").ordinal, is(3));

        assertThat(childrenByNames.get("p2_child1").parentId, is(retrievedParentIds.get(1)));
        assertThat(childrenByNames.get("p2_child1").ordinal, is(1));

        assertThat(childrenByNames.get("p2_child2").parentId, is(retrievedParentIds.get(1)));
        assertThat(childrenByNames.get("p2_child2").ordinal, is(2));

        assertThat(childrenByNames.get("p2_child3").parentId, is(retrievedParentIds.get(1)));
        assertThat(childrenByNames.get("p2_child3").ordinal, is(3));
    }

    @Test
    public void upsert_existing_parents_and_replace_children() {

        // setup

        var retrievedParentIds = seq(persistenceLayer.upsert(List.of(
                new UpsertByIdInTarget(1000).with(ParentEntity.NAME, "parent1")
                    .with(new UpsertChild(1).with(ChildEntity.FIELD_1, "p1_child1"))
                    .with(new UpsertChild(2).with(ChildEntity.FIELD_1, "p1_child2"))
                ,
                new UpsertByIdInTarget(2000).with(ParentEntity.NAME, "parent2")
                        .with(new UpsertChild(1).with(ChildEntity.FIELD_1, "p2_child1"))
                        .with(new UpsertChild(2).with(ChildEntity.FIELD_1, "p2_child2"))
        ), flow.build())).map(res -> res.getCommand().get(ParentEntity.ID)).toList();

        // action

        persistenceLayer.upsert(List.of(
                new UpsertByIdInTarget(1000).with(ParentEntity.NAME, "parent1")
                        .with(new UpsertChild(1).with(ChildEntity.FIELD_1, "p1_child1_new"))
                        .with(new DeletionOfOther<>(ChildEntity.INSTANCE))
                ,
                new UpsertByIdInTarget(2000).with(ParentEntity.NAME, "parent2")
                        .with(new UpsertChild(2).with(ChildEntity.FIELD_1, "p2_child2_new"))
                        .with(new DeletionOfOther<>(ChildEntity.INSTANCE))
        ), flow.build());
        ...
        assertThat(childrenByNames.keySet(), containsInAnyOrder("p1_child1_new", "p2_child2_new"));

        assertThat(childrenByNames.get("p1_child1_new").parentId, is(retrievedParentIds.get(0)));
        assertThat(childrenByNames.get("p1_child1_new").ordinal, is(1));

        assertThat(childrenByNames.get("p2_child2_new").parentId, is(retrievedParentIds.get(1)));
        assertThat(childrenByNames.get("p2_child2_new").ordinal, is(2));
    }
    ...
    class UpsertChild extends InsertOnDuplicateUpdateCommand<ChildEntity, ChildEntity.Ordinal> implements EntityCommandExt<ChildEntity, UpsertChild> {
        public UpsertChild(int ordinal) {
            super(ChildEntity.INSTANCE, new ChildEntity.Ordinal(ordinal));
        }
    }
    ...
    private void insertSomeChildWithOrdinal(int ordinal) {
        final int RANDOM_PARENT = 50012;
        dslContext.insertInto(CHILD_TABLE)
                .set(CHILD_TABLE.parent_id, RANDOM_PARENT)
                .set(CHILD_TABLE.ordinal, ordinal)
                .set(CHILD_TABLE.field1, "RANDOM_F1_" + ordinal)
                .execute();
    }
